...
function injectAuthModal(){
  ...
  if(typeof injectHoneypot === 'function') injectHoneypot(document.getElementById('auth-modal-signup'));
...
function openAuthModal(which){
  ...
  const signupPane = document.getElementById('auth-modal-signup');
  if(signupPane) signupPane.dataset.hpRenderedAt = Date.now();
}
...
async function handleSignup(){
  ...
  if(typeof honeypotTripped === 'function' && honeypotTripped(document.getElementById('auth-modal-signup'))){
    formMsg('signup-msg','Could not create the account — try again.'); return;
  }
